...
namespace Folke.Orm
{
    internal class SchemaUpdater
    {
        ...
        {
            var tables = assembly.DefinedTypes.Where(t => t.IsClass && (t.GetInterface(typeof(IFolkeTable)) != null || t.GetCustomAttribute<TableAttribute>() != null)).Select(x => connection.Mapper.GetTypeMapping(x.AsType())).ToList();
